...
class ReporterPlugin extends Tapable {
	...
	apply(compiler) {
		...
		self.compilerHooks.forEach(hookName => {
			...
			if (!self.hookStats.hasHook(hookId)) {
				self.hookStats.initHook(hookId);
			}

			// TODO handle args
			compiler.hooks[hookName].tap(self.REPORTER_PLUGIN, (...args) => {
				const hookData = self.hookStats.generateHookData(hookId);
				// Emit the log
		...
		compiler.hooks.done.tap(self.REPORTER_PLUGIN, stats => {
			...
			self.hookStats.incrementCount(hookId);
			/* @type {HookData} */
			const hookData = self.hookStats.generateHookData(hookId, stats);
			// Emit the log
	...
	onCompilation(compilation) {
		...
		self.compilationHooks.forEach(hookName => {
			...
			compilation.hooks[hookName].tap(self.REPORTER_PLUGIN, data => {
				// let hook = self.hookStats.getHook(hookId);
				self.hookStats.incrementCount(hookId);
				if (self.hookStats.shouldTrigger(hookId)) {
					/* @type {HookData} */
					const hookData = self.hookStats.generateHookData(hookId, data);
					self.hooks.info.call(hookData);
				}
			});
		});
